...
- scp un binaire buildé sur WSL2 vers un VPS (recette : checkout de `deploy/mainnet` + make SUR le VPS ; sur OVH, GLIBC 24.04 vs 22.04)
- `git pull` de `v2-privacy` sur un VPS de production — la production suit `deploy/mainnet` uniquement
...
**Branche de production :** les 4 VPS tournent sur la branche **`deploy/mainnet`** (`5e1559995` = `6e0d10d42` + checkpoint h=90000), **plus sur `v2-privacy`**. `v2-privacy` contient du code non déployé (travail post-quantique de septembre 2026, liboqs 0.16.0). **Toute mise à jour de production passe par `deploy/mainnet`, jamais par un `git pull` de `v2-privacy` sur un VPS.**

**Auth SSH (MAJ 12 juin 2026) :** key-only (ed25519) sur les 4 hôtes — plus de password. **Recette de déploiement canonique : git pull + make SUR le VPS, JAMAIS de scp de binaires depuis WSL2.** Raison : OVH est en Ubuntu 22.04 → un binaire buildé sur WSL2 (24.04, glibc 2.39) ne tourne PAS dessus (GLIBC incompatible) ; sur les Contabo (24.04) le build local au VPS reste plus sûr (mêmes libs dynamiques). Séquence : `ssh ubuntu@135.125.243.137` (ou `root@<ip-contabo>`) → `cd ~/hidering && git fetch && git checkout deploy/mainnet && git pull --ff-only` (jamais `v2-privacy`) → `cd build` (**attention : le build dir VPS est `~/hidering/build`, PAS `build/release` — brûlé le 12 juin sur OVH**) → `make -j$(nproc) daemon` (wallet : `make -j2` obligatoire partout, OOM `wallet2.cpp`) → `[sudo] systemctl restart hideringd.service` (`sudo` sur OVH uniquement). **Gotcha Contabo-2 (brûlé 12 juin 2026)** : son daemon mine (`--mining-threads 6`, dataset RandomX ~2,3 GB) + stack pool (Redis/PM2/nginx) → `cc1plus` OOM-killed sur les grosses TU (`core_rpc_server.cpp`) même en `-j1`. Séquence obligatoire sur Contabo-2 : **arrêter xmrig pendant tout le build** (mémoire) → `systemctl --job-mode=ignore-dependencies stop hideringd` → `make -j2 daemon` → `systemctl --job-mode=ignore-dependencies start hideringd` → relancer xmrig (le pool re-health-check tout seul, vérifier `curl localhost:8117/stats` → `daemon_ok`). `--job-mode=ignore-dependencies` est obligatoire : `hrg-wallet-rpc` a `Requires=hideringd`, un stop simple l'arrêterait aussi. **Redémarrer juste après un cycle de paiement** (intervalle 1800 s, cf. `payments_info.log`). **Vérification du binaire :** le libellé de `hideringd --version` ne change pas sans relancer cmake → vérifier un binaire avec `strings` plutôt qu'avec `--version`. Contabo-1 peut aussi OOM sporadiquement en `-j$(nproc)` → retry en `-j2`. *Ancienne recette scp + password (21 mai 2026) OBSOLÈTE.*
