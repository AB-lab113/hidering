...
### Étape 4 (PROCHAINE) — intégration Kyber768 dans la dérivation des stealth addresses (adresses BQ...)
Câbler Kyber768 (KEM) dans la dérivation d'adresse furtive pour le nouveau format d'adresse BQ... (`CRYPTONOTE_PQ_ADDRESS_PREFIX = 0x3C11`). Remplace/complète l'ECDH classique pour les adresses post-quantiques. Devra aussi résoudre les 3 caveats de l'Étape 3 (plumbing clé réelle, vraie HF version passée par le wallet, taille extra). Toujours additif et gardé par HFv16.
