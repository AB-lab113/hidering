...
**Dernière MAJ : 13 Septembre 2026 (**upgrade liboqs 0.15.0 → 0.16.0 PRISE** — le gate du 8 septembre honoré : vecteur figé rejoué, **4 groupes identiques bit pour bit**, 15/15 tests verts, arbre restauré et re-vérifié AVANT la décision. Raison lue dans le source des deux versions : chacune tire **un seul bloc de 32 o** avant la dérivation `ξ → (pk, sk)` **spécifiée FIPS 204** — `mldsa-native` a changé l'implémentation, pas le motif d'aléa. **Risque macOS ARM64 écarté sur les deux versions** (0.16.0 : les 3 backends partagent un `sign.c` au hash identique ; 0.15.0 : pas de backend aarch64 du tout, ARM tombe sur `ref` vs `avx2` sur x86_64, mais le prologue aléa→(rho,rhoprime,key) est **byte-identique**) — ⚠️ **par lecture, jamais mesuré sur ARM**. Submodule → `5a1a854b…` (0.16.0) ; `pq_vector_test` vaut désormais pour **les deux versions**, valeurs inchangées. **M-11 reste BLOQUANT pour HFv16** (disclaimer intact, pas de FIPS 140-3, pas de `keypair_derand` SIG) — l'upgrade ne fait que nous sortir de l'EOL. Correction : « aucun audit tiers » était faux (Trail of Bits, portions, 2024). Détail en fin de fichier, section « 13 Septembre 2026 ».) — Antérieur 12 Septembre 2026 (**spec 2e livrée — binding C-1 + S1b** : l'adresse BQ publie un engagement de 32 o vers une clé ML-DSA **par (sub)adresse** (format `marker|auth_ver|B|A|kem_pk|auth_commit` = 1282 o → **1770 car.**), l'entrée porte une autorisation **typée** (`auth_type|auth_blind|auth_pk|auth_sig`, pin wire **5512 o**), et le `bind_tag v2` relie les deux sous un aveuglement à sens unique de `ss` — **sans cet aveuglement, une seule clé révélée démasquerait toutes les sorties d'une subaddress**. **Résiduel CRIT-3 FERMÉ** (un émetteur quantique à qui l'on donne `x'` échoue sur (c) seul). Signature de compte `0x06` **supprimée** → budget `tx_extra` **2 → 7 sorties BQ**. **T2 imposée** par le wallet (R-a/R-b/R-c) ; le change d'une dépense BQ n'atterrit plus sur l'index primaire ni sur une sortie classique. Résidu §3.4 (l'émetteur choisit ce qu'il engage) **détecté au scan, jamais crédité**. 2 défauts latents corrigés au passage. 15 tests verts. Détail en fin de fichier. — Antérieur même jour : **CRIT-4 corrigé** — la racine des clés BQ était `m_spend_secret_key`, logarithme discret de la clé publique de dépense publiée dans l'adresse BQ : Shor livrait TOUT le matériel post-quantique du compte. Rapport de design `design_2c` d'abord, qui **réfute l'hypothèse intuitive** (« redériver depuis la seed sous un autre domaine de hash ») en établissant que **la seed 25 mots EST la clé de dépense** (`crypto.cpp:159-166`, `wallet2.cpp:1450-1453`) → il fallait de l'entropie nouvelle. **Décision R2a : deux mnémoniques** — `account_keys::pq_root`, 32 o indépendants avec leur propre seed 25 mots ; adresse B... inchangée pour une même seed classique ; restauration BQ sans seed BQ **refusée** (fail-closed, dans `wallet2::generate`) ; `.keys` BQ d'avant le correctif refusé au load ; `BOOST_CLASS_VERSION` 1→2 ; `pq_vector_test` régénéré **au seul niveau compte**, vecteurs brut-seed/par-sortie re-mesurés **identiques**. Aucune migration (HFv16 jamais activé). **Résiduel CRIT-3 émetteur quantique toujours ouvert** → S1b, avec C-1. Détail en fin de fichier, section « 12 Septembre 2026 ».) — Antérieur 11 Septembre 2026 (**subaddresses BQ livrées** — décision 4, option B3 : une paire ML-KEM par subaddress + tag de sélection aveuglé, commits `1d901e541`/`a88d5297c` ; **CRIT-3 corrigé** (`678c170b9`) — l'émetteur d'une sortie BQ pouvait la redépenser, check consensus **d2** (signature par la clé unique `x`), + deux défauts de l'hybride qui rendaient TOUTE tx hybride invalide ; **CRIT-4 ouvert** (racine PQ = clé de dépense, retrouvable par Shor). Détail en fin de fichier, section « 11 Septembre 2026 ».) — Antérieur 8 Septembre 2026 (nuit : **CRIT-2 — création monétaire** dans la dépense BQ transparente, trouvée en implémentant l'hybride : `txin_to_key_pq.amount` n'était lié à rien (les checks a–d prouvent la POSSESSION, pas la VALEUR) → une sortie BQ de poussière pouvait être déclarée à 1000 HRG. HFv16 jamais activé → aucun fonds réel exposé. Corrigé par le check **b2** (le masque est publié, `engagement == amount*H + mask*G`). **Transactions hybrides livrées** (décision 3, H2 : entrées transparentes = frais négatifs, `pseudoOuts` reste 1:1 avec les CLSAGs, ordre ring-avant-PQ imposé) → consolidation B.../BQ possible, et **plus privée** que la dépense BQ pure. Détail en fin de fichier. — Plus tôt le 8 : **cold-sign BQ RÉSOLU** via la table latérale `unsigned_tx_set` v4 — ciphertext ML-KEM transporté, jamais `pq_ss` ; jeux classiques byte-identiques en v3/`\005`, jeux PQ en `\006` pour qu'un ancien wallet REFUSE au lieu de mal signer ; **vecteur de non-régression liboqs figé sur 0.15.0**, upgrade 0.16.0 toujours NON faite. Détail en fin de fichier. — Plus tôt : revue post-audit du 7 septembre — Phases 1–3 : **MOYEN-4** RNG déterministe isolé par thread (`cda1842b2`, c'était un **use-after-free** reproductible, pas seulement une faiblesse d'aléa) ; **1c** cold-sign partiellement corrigé (`00a81d275`) avec la vraie rupture identifiée et documentée, décision de format en attente ; **M-11** blocage liboqs MAINTENU (0.16.0 sortie, notre 0.15.0 EOL, piège d'upgrade sur les adresses BQ dérivées) ; rapports de design 2a/2b **sans code, en attente de validation** ; doc : promesse « activation 24–48h » retirée, §6.2 transparence BQ ajouté, README réécrit, checkpoints synchronisés. Détail en fin de fichier, section « 8 Septembre 2026 ». Sections **A1–A4 / C2–C4** enfin documentées (elles étaient committées mais absentes de ce fichier). — Antérieur 6 Septembre 2026 : checkpoint mainnet h=80386 ancré + déployé — commit `6e0d10d42` sur v2-privacy : `ADD_CHECKPOINT2(80386, "92e4d8914fa54ec17342c4a983cef4ea049bfb4a99907f0eefcdf269d35eca41", "0xa818c8c876")` dans `src/checkpoints/checkpoints.cpp`. Hauteur choisie = tip−20 (tip seed1 OVH = h=80406 au moment de la capture ; bloc 80386 à `depth=21` → ≥20 confirmations, hors zone de réorg) ; hash + cumulative difficulty croisés via `get_block_header_by_height` sur le daemon seed1 OVH (chaîne hf 15, major_version 15). Déploiement git pull + `make daemon` + restart dans l'ordre **Contabo-1 → Contabo-3 → Contabo-2 → OVH** (≥1 nœud up en permanence), les 4 `active` + `sync=True` + alignés sur le même top hash (h≈80440, `11e0bdaa09d90dc4`). **Preuve d'enforcement** : OVH tourne en `--log-level 2` et a loggé `CHECKPOINT PASSED FOR HEIGHT 80386 <92e4d8914…eca41>` au boot ; les 3 autres (log-level 0) ne l'impriment pas mais sont synced au-delà de 80386 sous l'enforcement hash réel (depuis `934b43b01`). **Gotcha du jour** : les 4 VPS étaient plusieurs commits en retard (`2dc04c2e0`) → le `git pull` a amené AUSSI tout le travail PQC/HFv16 (34 fichiers, 1665 insertions) — **inerte** sur la chaîne live (hf 15 ; HFv16 à h=2 000 000) mais le build daemon devient un rebuild ~complet (pas un simple relink checkpoint) et beaucoup plus long. RAM adaptée par hôte : Contabo-1 `-j6` (8 GB libres), Contabo-3 `-j4`, Contabo-2 `-j2` (séquence `systemctl stop hideringd` → `make -j2 daemon` → `start`, gotcha OOM mining+pool, **pm2 `hidering-otc`/`hrg-pool` NON touchés** — PIDs inchangés, pool re-health-check OK), OVH `-j2` (4 GB, 4 cores, `sudo` requis pour systemctl, build dir `~/hidering/build`). Aucun OOM, aucun incident. Build `daemon` uniquement (un checkpoint ne touche pas wallet2.cpp). — Antérieur 19 Juin 2026 (checkpoint mainnet h=25000 ancré + déployé — commit `2dc04c2e0` sur v2-privacy : `ADD_CHECKPOINT2(25000, "59badae1…dd188", "0x6adc13c1a9")` dans `src/checkpoints/checkpoints.cpp`, hash + difficulté cumulative croisés via RPC du daemon seed1 OVH (chaîne ~h25500). Déploiement git pull + `make daemon` + restart sur les 4 VPS, tous `active` et alignés sur le même top hash (h=25527, `fc963095ba868e23`, `sync=True`). Build `daemon` uniquement (un checkpoint ne touche pas wallet2.cpp). **Gotcha reconfirmé : Contabo-2 OOM-kill `cc1plus` sur `core_rpc_server.cpp` même en `-j2`** (mining + stack pool concurrents) → séquence `systemctl stop hideringd` → **`make -j1 daemon`** → `systemctl start` ; pool re-health-check OK (network height = daemon). OVH/Contabo-1/-3 ont buildé en `-j$(nproc)` sans souci. — Antérieur 12 Juin 2026 (checkpoint mainnet h=20000 ancré + déployé — commit `df7211b2d` sur v2-privacy : `ADD_CHECKPOINT2(20000, "5f7832fd…a54e", "0x56176a690c")` dans `src/checkpoints/checkpoints.cpp`, hash croisé seed1 OVH + seed2 Contabo. Déploiement git pull + make daemon + restart sur les 4 VPS (OVH → Contabo-1/-2/-3), tous `active` à HEAD `df7211b2d`, seeds alignés h=20480. NB : l'enforcement des checkpoints est RÉEL depuis le commit `934b43b01` (6 juin 2026, réactivation du hash-check `check_block()` court-circuité relevé par l'audit) — appelé main chain (`blockchain.cpp:4175`), alt blocks (`:1934`) et load DB (`:4732`). Gotcha build OVH : le build dir y est `~/hidering/build`, pas `build/release`. Même jour, tâches M-x : **M-5** seeds Tor/i2p Monero purgés (`eee5dec50`, cf. FICHIERS CRITIQUES net_node.inl) ; **M-13** RPC pool Contabo-2 confirmé localhost-only (doc, cf. INFRASTRUCTURE VPS) ; **M-3** `.keys` sans MAC = risque accepté, fix format-breaking refusé (cf. SÉCURITÉ) ; **M-6** DNSSEC seeds CORRIGÉ — zone hidering.org signée + DS au registre, drop `avail && !valid` câblé dans `get_dns_seed_nodes` (`fbf0e0600`), déployé 4 VPS (cf. SÉCURITÉ) ; **M-9** CVE-2024-38517 rapidjson = faux positif, déjà fixée dans le pin (cf. CVE / FAUX POSITIFS) ; **seed3/seed4 NXDOMAIN** retirés de `m_seed_nodes_list` (`net_node.h`, commit `6047bb49e`, déployé 4 VPS — ré-ajouter seulement si les records A sont créés). — **13 Juin 2026 (suite tâches M-x + seeds)** : **M-7** padding TX 2500 o = plancher ≡ taille fixe de facto pré-HFv16 → NON-PROBLÈME, fermé (commit `a220b960d`, cf. SÉCURITÉ) ; **M-10** provenance liboqs : le SHA submodule épinglé (`97f6b86` = tag `0.15.0`) EST l'ancre d'intégrité (git content-addressé ; OQS ne signe NI tag NI checksum) → procédure de vérif `rev-parse`/`tag --points-at` documentée, risque FAIBLE car PQC inerte jusqu'à HFv16 (commit `174c4beac`, cf. SÉCURITÉ) ; **M-11** liboqs = lib de prototypage (disclaimer README « not for production » toujours présent en 0.15.0 ; algos ML-KEM/ML-DSA eux FIPS-finaux) → risque ACCEPTÉ car PQC inerte jusqu'à HFv16, MAIS **réévaluation maturité BLOQUANTE avant activation HFv16** (commit `174c4beac`, cf. SÉCURITÉ) ; **seed3/seed4 RÉ-AJOUTÉS** à `m_seed_nodes_list` (`net_node.h`, commit `99abd9dc4` — records A créés : seed3→Contabo-2 `207.180.214.164`, seed4→Contabo-3 `167.86.74.202`, smoke test 4 dns_threads OK, déployé 4 VPS, mesh P2P complet vérifié). **GUI : AppImage Linux v2.0.2-gui REBUILDÉ** (était un report v2.0.1) — `Hidering_Wallet-v2.0.2-x86_64.AppImage` 57 MB sha256 `54ea3e93…c27f759`, uploadé sur la release + ancien asset v2.0.1 supprimé ; `docs/index.html` MAJ (commit `d24005978`), live hidering.org vérifié (cf. RELEASE v2.0.2-gui). — **14 Juin 2026 (M-4 fermé)** : clés BQ (ML-DSA-65 + ML-KEM-768) dérivées **déterministe­ment de la spend key** (commit `1473779b6`) → restore depuis seed 25 mots reproduit l'adresse BQ (auparavant clés OQS aléatoires → fonds BQ perdus au restore). Nouveau `crypto::pqc::pqc_keygen_from_seed` : sous-seeds SHAKE256 domain-séparés (`"HRG_PQ_KEM_v1"`/`"HRG_PQ_DSA_v1"` ‖ spend_sk, one-way) → ML-KEM via `OQS_KEM_keypair_derand` (seed 64 o), ML-DSA — **pas de keygen dérandomisé dans liboqs 0.15.0** — via RNG SHAKE256 installé sur le hook `randombytes` global d'OQS (`OQS_randombytes_custom_algorithm`, mutex, RNG système restauré avant release). `generate_pq_keys` (`account.cpp`) appelle désormais `pqc_keygen_from_seed(&m_spend_secret_key, 32, …)` ; **signature inchangée → 0 call site modifié** (wallet2.cpp inclus). Gardé HFv16, chaîne live (hf 15) intacte ; anciens wallets BQ aléatoires changent (accepté : déjà invalidés par la migration FIPS Étape 9, HFv16 inactif). Tests : `pq_keygen_test::test_bq_keygen_is_deterministic` (restore reproduit BQ, seed distinct → BQ distinct) PASS, `pq_persistence_test` PASS, build `obj_cncrypto`+`cryptonote_basic` clean. **Backlog MOYEN 10 juin = 12/13 fermés, seul M-8 ouvert.** NB encore ouvert (hors M-4) : dérivation des subaddress BQ. Voir SÉCURITÉ « État du backlog » + [[project_mx_backlog_open_items]]. — Antérieur 4 Juin : Phase 5 Étape 6 DONE — keygen BQ + préfixe mnemonic "BQ" + détection des sorties BQ, commit `62cb02066` sur v2-privacy. Préfixe `0x3C11`→**62** + octet marqueur `0x33` dans le payload → l'adresse commence vraiment par "BQ" (prouvé : un préfixe seul ne le peut pas ; 62 collisionne avec le préfixe subaddress, désambiguïsé par taille payload 64 vs 1249 o). `generate_pq_keys`/`get_pq_address_str`/`get_keys_nonconst` ; `wallet2::generate(use_pq=false)` ; détection par dé-tweak `P−t·G` avant `is_out_to_acc_precomp` gardée par `pq_keys`. Build 0 nouveau warning, regtest 2 blocs clean, `pq_keygen_test.cpp` PASS. Items ouverts : `pq_keys` non sérialisé (clés BQ éphémères/session), subaddress BQ, cold-sign hf, bump `MAX_TX_EXTRA_SIZE` à HFv16. Voir POST-QUANTIQUE Étape 6. — Antérieur : Étape 5 DONE — format d'adresse BQ... + plumbing des clés Kyber768 côté wallet, commit `b77639979` sur v2-privacy, gardé par HFv16 + flag `is_pq`, chaîne hf 15 byte-identique. `account_public_address` porte désormais une clé Kyber768 optionnelle de 1184 octets (absente des deux maps de sérialisation → format B... inchangé) ; encaps utilise la vraie clé destinataire (throwaway Étape 4 retiré) ; caveat 2 résolu sur les 2 call sites online. Items ouverts Étape 5 : (1) rien ne set `account_keys.pq_keys` encore → keygen BQ + subaddress à faire (le tweak de scan est donc dead code), (2) préfixe `0x3C11` ne rend PAS "BQ" en base58 (rend "RLF..." sur payload 1184 o) → tuning préfixe mnemonic à faire, (3) cold-signing HFv16 nécessite `hf_version` dans les unsigned-tx exportées. Voir POST-QUANTIQUE Étape 5. — Antérieur 4 Juin : Étape 4 DONE (dérivation stealth BQ... via Kyber768, commit `0c489dbdc`). — Antérieur 3 Juin : Phase 4D GUI COMPLETE — release multi-plateformes `v2.0.2-gui` publiée : macOS ARM64 + Windows x64 buildés par CI GitHub Actions (workflows `build-{macos,windows}-gui.yml` sur `AB-lab113/hidering-gui` master) + Linux AppImage report de v2.0.1-gui. 4 bugs latents mac/win corrigés (Deploy.cmake + Info.plist target rebrand, gitlink monero pré-liboqs, link Windows ole32/shell32). Site hidering.org MAJ : GUI 3 OS, roadmap Phase 4H GUI Desktop = Complete, Phase 5 PQ = In Progress, mentions exchanges retirées. Voir RELEASE v2.0.2-gui + section HIDERING WALLET GUI. — Antérieur 23 Mai : Release Linux v2.0.0-gui du Hidering Wallet GUI publiée sur github.com/AB-lab113/hidering-gui/releases/tag/v2.0.0-gui : AppImage 54 MB self-contained + tar.gz 11 MB binaire dyn. Build reproductible via `scripts/build-appimage.sh` committé à HEAD `9c737436`. Voir section HIDERING WALLET GUI ci-dessous. — Plus tôt 23 mai : fork + rebrand + push initial. 21 mai : checkpoint mainnet h=5000 + 4-VPS inventory. 20 mai : harmonisation `--restricted-rpc`. 19 mai : MIGRATION INFRA Flux → VPS dédiés.)**
...
- Tags publiés :
  - **v2.0.3** → commit `c9b4f1d73` — dernière publique ; bundles complets, checkpoint h=25000 ; version bump 2.0.2→2.0.3
...
- **M-6 : DNSSEC sur la résolution des DNS seeds — CORRIGÉ (12 juin 2026, commit `fbf0e0600`).** La zone `hidering.org` est désormais **signée + DS publié au registre `.org`** (DS `52475 8 2 52B6…9F205C75`, vérifié à l'autoritaire ET via Quad9/1.1.1.1 ; flag `ad` confirmé sur seed1/seed2). Le drop `if (avail && !valid) addr_list.clear()` est câblé dans la lambda de `get_dns_seed_nodes` (`net_node.inl`) : une réponse seed dont la signature est activement bogus est rejetée (MWARNING + fallback IP seeds) ; une résolution « insecure » (`avail=false`, ex. cache résolveur pas encore à jour) garde le comportement permissif → dégradation gracieuse, jamais de seeds droppés à tort. Smoke test local : daemon patché résout seed1/seed2 (1 résultat chacun, zéro warning DNSSEC) à travers Quad9. Déployé sur les 4 VPS le 12 juin 2026. **Gotchas de la vérification (brûlés en live)** : (1) le panel OVH peut afficher « publié » bien avant le registre — la vérité est `dig +short DS hidering.org @9.9.9.9` non vide + flag `ad`, et entre la base registre (whois `signedDelegation`) et la zone `.org` servie il peut s'écouler des heures ; (2) un grep de sortie `dig` sur `IN DS` matche la section QUESTION → toujours utiliser `dig +short` pour tester la présence d'un enregistrement ; (3) requêtes directes aux autoritaires REFUSED depuis le réseau WSL2 → vérifier depuis un VPS. *Décision antérieure (caduque) : risque accepté tant que la zone n'était pas signée — l'enforcement sans DS aurait été du théâtre de sécurité.* État : `get_dns_seed_nodes` (`net_node.inl:~798`) appelle `get_ipv4(addr, avail, valid)` et **ignore** les deux flags DNSSEC (le `TODO: care about dnssec avail/valid` est hérité d'upstream — **parité Monero release-v0.18 vérifiée**, même comportement). Le résolveur (`dns_utils.cpp`) calcule bien `dnssec_valid` via libunbound + ancres, mais tourne en `val-permissive-mode` (choix HIDERING documenté in-code : des forwarders qui strippent RRSIG/NSEC feraient passer toute zone non signée pour bogus → NXDOMAIN). **Point décisif : la zone `hidering.org` n'est PAS signée** (vérifié 12 juin : pas de DS au registre, pas de DNSKEY, pas de RRSIG, pas de flag `ad`) → `dnssec_available=false` sur toutes les résolutions seed → n'importe quel enforcement code serait un no-op sécurité (un empoisonneur d'une zone non signée produit des réponses non signées indistinguables des légitimes), tout en risquant des faux positifs « bogus » côté forwarders (exactement le problème qui a motivé le mode permissif) → casserait la découverte des seeds pour les nouveaux nœuds, pour zéro gain. Mitigation actuelle : fallback IP en dur (`get_ip_seed_nodes`) + un faux seed ne peut pas forger la chaîne (NETWORK_ID + PoW + checkpoints hash-enforced) — l'attaque se limite à de l'eclipse au bootstrap. **Chemin de remédiation (dans l'ordre, le jour où on veut le faire)** : (1) signer `hidering.org` chez OVH (DNSSEC managé + DS publié), (2) vérifier `dig +dnssec seed1.hidering.org` → flag `ad`/RRSIG présents via les résolveurs de `DEFAULT_DNS_PUBLIC_ADDR`, (3) seulement alors câbler le drop `if (avail && !valid) addr_list.clear()` dans la lambda de `get_dns_seed_nodes` (~5 lignes) — l'étape 3 sans les étapes 1-2 est du théâtre de sécurité.
...
- **M-7 : padding TX 2500 o = plancher (floor) et non taille fixe — NON-PROBLÈME PRATIQUE, fermé (13 juin 2026).** Verdict : **plancher fonctionnellement équivalent à une taille fixe pour toutes les TX pré-HFv16.** Le finding notait, à juste titre au sens strict, que le padding F-4 est un *floor* (pad-up) et non une normalisation à taille fixe : le code (`cryptonote_tx_utils.cpp:547-576`, fix F-4 du 6 juin) ne pad QUE quand `tx.extra.size() < 2500` (`if (tx.extra.size() < HIDERING_EXTRA_PADDING_TARGET) { push TX_EXTRA_TAG_PADDING + zéros }`) — il n'y a **jamais de troncature** si l'extra dépasse déjà 2500. **Pourquoi c'est sans effet en pratique** : sur la chaîne live (gardé `if (hf_version < HF_VERSION_PQ)`, ligne 568 — hf 15, le seul chemin actif), le `tx_extra` d'une TX normale ne contient que la clé publique TX (~33 o) + quelques champs optionnels (clés pub additionnelles 33 o/sortie, payment-id chiffré 9 o, nonce) → il est **toujours très en dessous de 2500 o** → le padding s'applique **systématiquement** et porte chaque TX exactement à 2500 o → de facto **fixe** pour 100 % des TX réalistes. Atteindre le plancher exigerait une TX pathologique à ~75 sorties (75×33 ≈ 2475 o de clés pub additionnelles) — hors de tout usage réel (et même alors : pas de fuite de taille variable au-delà, juste l'absence de pad). **Le seul cas légitime où l'extra dépasse 2500 o est le PQC** (BQ, ML-DSA-65 pk+sig 5261 o, ML-KEM-768 CT 1088 o/sortie BQ) — déjà **géré séparément sous HFv16** : le bloc de padding est OMIS sur le chemin PQ (audit H2, doc in-code lignes 562-567) car le 0x00 padding greedy et la sig ML-DSA terminale (0x06) sont mutuellement exclusifs ; les champs PQ volumineux fournissent eux-mêmes l'obscurcissement de taille. **Conclusion : aucune modification de code requise.** Le « plancher » est le comportement correct (ne jamais tronquer un extra légitimement gros) et, sur la seule chaîne active, indiscernable d'une taille fixe. À ne réexaminer que si un cas d'usage pré-HFv16 produisait régulièrement des `tx_extra` > 2500 o (aucun connu).
...
- **Release multi-plateformes v2.0.2-gui publiée 3 juin 2026** : https://github.com/AB-lab113/hidering-gui/releases/tag/v2.0.2-gui — **1ère release GUI multi-OS, buildée par CI GitHub Actions** (workflows `.github/workflows/build-{macos,windows}-gui.yml` ajoutés sur `master`, commit `5caa0431` ; CI PAS bloquée par billing, cf. [[project_ci_build_workflows]]). Tag au commit `5caa0431`, backend submodule `monero` bumpé à v2-privacy HEAD `55815b345` (v2.0.2 + PQC/liboqs). Assets :
  ...
  - `Hidering_Wallet-v2.0.2-x86_64.AppImage` — 57 MB (59 025 912 o) — sha256 `54ea3e936bb6a6b650a5ede347218aef3cd6696bae44532d36ff41eaec27f759` (**rebuildé 13 juin 2026** depuis les sources v2.0.2-gui — repo `~/hidering-gui` master `5caa0431`, via le script canonique `./scripts/build-appimage.sh v2.0.2` : Release + `-DMANUAL_SUBMODULES=ON`, `make -j2`, bundling linuxdeploy + plugin-qt + patchelf — **PAS linuxdeployqt, cassé sur ce stack** ; smoke test offscreen `--help` OK. Remplace le report v2.0.1 ; l'ancien asset `Hidering_Wallet-v2.0.1-x86_64.AppImage` + son `.sha256` supprimés de la release. `docs/index.html` MAJ — nom de fichier + lien + SHA256 + note (commit `d24005978`), live vérifié sur hidering.org.)
...
### Étapes A1–A4 + C2–C4 — dépense BQ transparente, de bout en bout ✅ (25–26 Juin 2026)
*(Section ajoutée le 8 septembre 2026 : ce travail était committé mais non documenté ici — le prochain audit doit pouvoir le trouver.)*

Le manque était le suivant : les Étapes 1–8 décrivent la **création** de sorties BQ, mais rien ne décrivait comment on les **dépense**. C'est l'objet de A1–A4. Décision d'architecture retenue : **Option 2 « transparente »** — une entrée BQ ne peut pas porter d'engagement de Pedersen, donc une dépense BQ est une tx **v2 `RCTTypeNull` à montants révélés**, équilibrée arithmétiquement. Tout est gardé `hf_version >= HF_VERSION_PQ` ; la chaîne live (hf 15) et les B... classiques ne bougent pas.

- **A2 — structures + validateur** (`7d53cb940`, 25 juin) : nouveau type d'entrée `txin_to_key_pq` (tag variant `0x3`) + `TX_EXTRA_TAG_PQ_BIND` (`0x08`). Le validateur applique 4 checks : **(a)** la sortie référencée existe et n'est pas dépensée, **(b)** le `real_output_key` révélé correspond à la clé on-chain, **(c)** le bind tag publié à la CRÉATION de la sortie engage `real_output_key` vers la clé ML-DSA-65 par sortie fournie, **(d)** la signature ML-DSA-65 vérifie sur le prefix hash. Double-dépense via **key image synthétique** (pas de nouvel index DB). Voir [[project_phase5_a2_pq_transparent_input]].
- **A3 — construction côté dépense + fix H-5** (`be593e203`, 26 juin) : `construct_tx` sait bâtir et signer un `txin_to_key_pq` ; check **(e)** conservation monétaire sur les montants révélés ; `get_tx_fee` calcule le frais depuis ces montants. **H-5** : le scanner décapsule désormais **tous** les ciphertexts ML-KEM d'une tx (`get_pq_output_shared_secrets` au pluriel) et retient celui qui matche. `recover_pq_spend_secret` re-dérive le secret partagé au moment de la dépense (aucun changement de sérialisation de `transfer_details`). **Restriction : tx tout-PQ uniquement** — l'hybride PQ+ring est refusé (voir rapport 2a). Voir [[project_phase5_a3_pq_transparent_spend]].
- **A3b — mempool** (`ddcfac2e9`) : chemin d'acceptation mempool pour les tx transparentes BQ (les gates `check_tx_semantics` / ladder RingCT rejetaient `RCTTypeNull`).
- **A4 — activation wallet + e2e complet** (`03e34a2ad`, 26 juin) : 3 verrous levés. (1) La signature ML-DSA-65 **de compte** devient **opt-in** — émise/exigée uniquement pour une dépense transparente BQ, donc un wallet B... classique transacte à HFv16 **sans** `pq_dilithium`. (2) Pas de changement de code pour le refresh : le rejet observé était la fenêtre de hauteur mainnet sur une chaîne regtest → `--allow-mismatched-daemon-version`. (3) `simplewallet --bq-wallet` crée un wallet ML-KEM-768 + ML-DSA-65 (`address bq`), et `get_account_address_from_str` route le payload 1249 o / préfixe 62 vers le parser BQ → `transfer BQ...` fonctionne. **E2E prouvé sur une seule session regtest** (l'LMDB regtest ne survit PAS à un restart) : B→BQ→détection→dépense→confirmation, avec sur la chaîne une vraie `txin_to_key_pq` en v2 `RCTTypeNull`.
- **C2–C4 — suivi local des sorties BQ dépensées + tests formels HFv16** (`b72425afe`).

### 7 Septembre 2026 — audit + 2 correctifs
- **CRIT-1 (perte de fonds)** — `6044493cf`. Une dépense BQ publie ses sorties avec des **montants non nuls** (le zeroing `vout[i].amount = 0` est sur la branche RingCT, jamais atteinte par la branche transparente), donc LMDB les indexait dans le bucket `amount`, alors que `check_tx_inputs` résout une entrée PQ dans le **bucket 0**. Résultat : **toute sortie BQ créée par une dépense BQ — le change en premier — était définitivement indépensable.** Invisible à l'e2e A4, dont la seule sortie BQ venait d'une tx RingCT B...→BQ... (montants zéroés → bucket 0 → le seul cas qui marchait). Fix : normaliser la **représentation en base**, pas le format wire — les sorties BQ transparentes sont stockées en bucket 0 comme sorties rct avec un engagement à masque identité `zeroCommit(amount)`, exactement le traitement Monero des coinbases v2. + gating **HAUT-2**.
- **JSON invalide** — `398e84f92`. La macro de sérialisation de `txin_to_key_pq` appelait `serialize_blob` sans `ar.tag("dsa")` → le blob ML-DSA de 5261 o était écrit sans clé ni séparateur, rendant `get_transactions.as_json` **non parsable** pour toute dépense BQ (explorateurs et tooling cassés). Le writer rapidjson, lui, était correct — seul le chemin epee était touché.

### 8 Septembre 2026 — revue post-audit (Phases 1–3)
- **MOYEN-4 — RNG déterministe non isolé** ✅ `cda1842b2`. Le flux SHAKE256 qui pilote le keygen ML-DSA (nécessaire : liboqs n'expose **toujours pas** de keygen dérandomisé pour SIG, vérifié jusqu'en 0.16.0) était installé sur le hook `randombytes` **process-global**, et `g_det_rng_mutex` ne sérialisait `pqc_keygen_from_seed` que **contre lui-même**. Tout autre thread entrant dans liboqs pendant la fenêtre puisait dans le flux déterministe d'autrui → (1) clés « aléatoires » dérivées de la spend key d'un autre wallet, (2) désynchronisation du flux → adresse BQ non reproductible depuis la seed (M-4 re-cassé), (3) **use-after-free** : le contexte SHAKE est une variable de pile du thread dérivant. Le nouveau test **segfault systématiquement** contre l'ancien code (gdb : `det_rng_fill` appelé depuis `..._MLKEM768_..._keypair` sur un autre thread, pendant qu'un 3e attend le mutex). Fix : hook installé **une seule fois** par process (`std::call_once`), dispatch sur un pointeur **thread-local** ; mutex supprimé (les dérivations concurrentes ne se gênent plus). Effet de bord corrigé au passage : le `switch_algorithm(system)` de nettoyage basculait tout le process d'OpenSSL RAND_bytes vers `/dev/urandom`, définitivement, dès le 1er keygen BQ. **Test : `src/crypto/pq_rng_isolation_test.cpp`.** NB : `pqc_keygen_output_dsa` passe par ce chemin → c'est le **chemin chaud** d'une dépense BQ, pas un cas théorique.
- **1c — cold-sign** ✅ partiel `00a81d275`. `account_boost_serialization.h` sérialise désormais `pq_keys`/`pq_dilithium` (versionné, `BOOST_CLASS_VERSION(account_keys, 1)`). **MAIS le diagnostic de l'audit était faux** : ce sérialiseur **n'a aucun appelant** (0 instanciation de template, vérifié par symboles ; son voisin `account_public_address` en a, via le cache wallet). Un cold-signer charge ses clés de son propre `.keys`. **La vraie rupture** : `tx_source_entry::{is_pq,pq_ss}` et `tx_destination_entry::{is_pq, addr.pq_kyber_pk}` ne sont **pas sérialisés** dans le fichier unsigned-tx → une dépense BQ exportée arrive en ressemblant à une dépense ring classique. **Non corrigé : décision de format en attente** → `docs/audit/coldsign_bq_gap_2026-09-08.md`.
- **M-11 (liboqs)** : blocage **MAINTENU**. Disclaimer « not for production » toujours présent upstream ; aucune validation FIPS 140-3 ; **0.16.0 est sortie (9 juillet 2026) et OQS ne supporte que la dernière release → notre 0.15.0 est EOL**. Aucun correctif 0.16.0 ne touche ML-KEM-768/ML-DSA-65. ⚠️ **Piège d'upgrade** : 0.16.0 bascule le backend ML-DSA sur `mldsa-native` ; comme notre dérivation BQ pilote le hook RNG, un changement du motif de lectures d'aléa **changerait l'adresse BQ dérivée d'une même seed** → figer un vecteur seed→clés sur 0.15.0 et le rejouer AVANT tout bump. → `docs/audit/M-11_liboqs_maturity_2026-09-08.md`.
- **Rapports de design (Phase 2, aucun code)** : `docs/audit/design_2a_hybrid_pq_ring_balance.md` (une entrée transparente = **frais négatif** dans la balance RingCT ; l'hybride serait **plus privé** que le chemin BQ actuel car les sorties redeviennent masquées) et `docs/audit/design_2b_bq_subaddresses.md` (ML-KEM n'est pas homomorphe → une paire par subaddress ; scan sauvé par un **tag de sélection aveuglé** de 8 o ; le binding C-1 n'est pas impacté).
- **Doc** : whitepaper — retrait de la promesse « activation en 24–48h » (fausse) + nouveau §6.2 sur la **transparence des dépenses BQ** ; **README entièrement réécrit** (c'était encore le README Monero brut) ; checkpoints synchronisés (7 ancres) ; whitepaper.html 1.4→1.4.2 ; index.html GUI v2.0.3→v2.0.2-gui.
- **Tests périmés corrigés** (`d90f19730`) : `pq_address_test` et `pq_keygen_test` affirmaient que `get_account_address_from_str` **rejette** une adresse BQ — l'inverse du comportement livré par A4 (26 juin). Ils échouaient depuis, sans que personne ne le voie. **Suite standalone complète = 8 tests, tous verts** : `pqc`, `pq_address`, `pq_keygen`, `pq_persistence`, `pq_bind`, `pq_spend`, `pq_rng_isolation`, `pq_coldsign`.

### 8 Septembre 2026 (suite) — cold-sign BQ v4 + gate liboqs
Décisions prises après le rapport de la revue post-audit, et exécutées le même jour.

- **Cold-sign BQ — RÉSOLU** (`2c6b82e85`). La rupture identifiée le matin (le fichier unsigned-tx ampute `tx_source_entry::{is_pq,pq_ss}`, `tx_destination_entry::is_pq` et `addr.pq_kyber_pk`) est corrigée par une **table latérale `unsigned_tx_set::pq_data`, écrite à partir de la v4**. Décisions retenues : (1) table latérale plutôt que champs ajoutés aux deux structs — celles-ci sont sérialisées **positionnellement** sans version propre, donc y ajouter un champ casserait le format du cold-sign **classique** pour tous les binaires v2.0.3 publiés ; (2) transport du **ciphertext ML-KEM-768, jamais `pq_ss`** — la machine froide re-dérive le secret avec ses propres `pq_keys`, donc **un fichier de transfert volé ne confère aucun pouvoir de dépense BQ**, ce qui est tout l'intérêt de signer hors-ligne.
  - **Gotcha central, non anticipé par l'analyse initiale** : le `VERSION_FIELD` de la structure **ne protège pas** un ancien wallet — il lirait version 4, traiterait ce qu'il connaît et **ignorerait le `pq_data` en fin de flux**, donc signerait une dépense BQ comme une dépense ring classique. Le seul élément qu'un binaire publié rejette est **l'octet de version au niveau FICHIER** → `UNSIGNED_TX_PREFIX_PQ` = `\006` pour un jeu porteur de PQ, `\005` conservé pour les jeux classiques.
  - **Le varint de version est émis à la main** (pas `VERSION_FIELD(4)`) : `VERSION_FIELD` réécrirait la version de **tous** les jeux, y compris classiques. On n'émet 4 que s'il y a du matériel PQ à décrire → **un transfert classique se sérialise exactement comme avant**.
  - `recover_pq_spend_secret` gagne un `ct_out` optionnel ; `tx_source_entry` gagne un `pq_ct` (non sérialisé) ; nouveau `wallet2::restore_pq_construction_data` (public, appelé par `sign_tx`) fait la moitié froide : décapsule chaque CT transmis et restaure les clés ML-KEM des destinations + du change.
  - **Test `src/crypto/pq_coldsign_v4_test.cpp`** (linke `libwallet`, exerce le VRAI chemin `dump_tx_to_str` / `parse_unsigned_tx_from_str` / `restore_pq_construction_data`) : jeu classique toujours en v3 sous `\005` (non-régression B... explicite) ; round-trip BQ avec secret partagé recalculé **identique** côté froid ; bind tag (check c) + signature (check d) reproduits ; **aucun secret dans les octets sérialisés** (`pq_ss`, sk ML-KEM, sk ML-DSA, spend sk, view sk) **avec un contrôle positif** qui fuit délibérément `pq_ss` et exige que la même recherche l'attrape — sans quoi « absent » ne prouverait rien ; octet de version inconnu refusé.
  - **Hors périmètre, inchangé** : `signed_tx_set` ne rapatrie pas les données PQ. La transaction signée est complète, donc la soumission n'est pas affectée ; seul `ptx.dests` afficherait un destinataire BQ sous sa forme B... dans l'historique du wallet chaud.
- **Vecteur de non-régression liboqs FIGÉ** (`5bb9910b5`) — `src/crypto/pq_vector_test.cpp`, généré sur 0.15.0. Verrouille : seed brut → `pqc_keygen_from_seed` (isole liboqs de la dérivation Monero) ; clé de restauration de test → **adresse BQ complète** (empreinte Keccak des 1725 caractères) + empreintes des pk **et** sk ML-KEM-768/ML-DSA-65 ; **clé ML-DSA par sortie** (chemin chaud d'une dépense BQ — si elle bouge, les sorties BQ déjà créées deviennent indépensables) ; bind tag. **NB : `pq_keygen_test` ne couvrait PAS ce risque** — il compare deux dérivations dans un **même** build, ce qui reste vrai même si toute la dérivation se décale entre versions. **L'upgrade 0.16.0 n'a PAS été faite** ; procédure et point de décision dans `docs/audit/liboqs_0.16.0_upgrade_gate.md` (rouge = upgrade non prenable telle quelle, il faudrait une dérivation versionnée ; ne JAMAIS réécrire les valeurs attendues pour faire passer le test). Sortie de crise à terme : implémenter nous-mêmes la dérivation FIPS 204 `ξ → (pk, sk)` pour cesser de dépendre du comportement RNG interne de liboqs.
- **Suite standalone = 10 tests, tous verts** (ajouts : `pq_vector_test`, `pq_coldsign_v4_test`). `HF_HEIGHT_PQ` inchangé à 2 000 000 ; chaîne live et cold-sign B... intacts.

### 8 Septembre 2026 (fin) — CRIT-2 (inflation) + transactions hybrides
- **CRIT-2 — création monétaire dans la dépense BQ transparente** ✅ `bab8ca00e`. **Trouvé en implémentant l'hybride.** `txin_to_key_pq.amount` n'était lié à RIEN : les checks (a)–(d) prouvent que le dépensier **possède** la sortie (clé révélée, bind tag, signature ML-DSA) mais **aucun ne dit ce qu'elle VAUT**, et le check (e) équilibre la tx *sur ce nombre*. Le propriétaire d'une sortie BQ de poussière pouvait donc déclarer `amount = 1000 HRG` et créer 1000 HRG de sorties — tous les checks passent. **Portée : HFv16 seulement, jamais activé → aucun fonds réel exposé.** Le commentaire in-code notait déjà « in.amount is attacker-supplied » à propos du bucket LMDB ; la conclusion n'avait pas été tirée pour la valeur. **Fix : check consensus (b2)** — `txin_to_key_pq` porte désormais le masque de la sortie et le validateur exige `engagement on-chain == amount*H + mask*G`. Couvre les deux origines d'une sortie BQ (masque ECDH pour une création RingCT classique ; masque identité pour une création par dépense transparente, cf. CRIT-1). Le wallet connaissait déjà le bon masque (`td.m_mask`). Zéro fuite (le montant est déjà public dans cette forme de tx), zéro coût de compat (HFv16 jamais actif). **Leçon** : autorisation ≠ valeur ; « révélé » ≠ « vérifié ». Voir `docs/audit/CRIT-2_pq_input_amount_unbound_2026-09-08.md`.
- **Hybride PQ+ring (décision 3, structure H2)** ✅ même commit. Une tx peut mélanger entrées BQ transparentes et entrées ring → **consolidation B.../BQ enfin possible**. **Algèbre** : une entrée transparente n'a pas d'engagement, mais le vérificateur peut en recalculer un depuis des données publiques avec masque nul (`a·H`) → `Σ pseudoOuts + Σ a_pq·H == Σ outPk + frais·H`, c'est-à-dire **les entrées transparentes = des frais négatifs**. `pseudoOuts` reste indexé sur les seules entrées ring, 1:1 avec les CLSAGs → **aucune table d'indirection dans le code de vérification consensus** (H1 rejetée pour ça : une erreur d'indexation y crée de la monnaie). Terme porté par `rctSigBase::pq_transparent_in`, **non sérialisé**, reconstruit depuis `tx.vin` par le vérificateur (comme `message`/`mixRing`) et **fail-closed** : à 0 par défaut, une tx hybride qui atteindrait un chemin oubliant de le peupler est REJETÉE. **Règle d'ordre consensus : toute entrée ring précède toute entrée PQ** (c'est ce qui préserve l'indexation 1:1 ; la règle « sorted ins » v7 ignore les types non-`txin_to_key`, donc elle reste satisfaite entre entrées ring).
  - **Effet contre-intuitif** : une tx hybride est **PLUS privée** qu'une dépense BQ pure — celle-ci est `RCTTypeNull` avec **tous** les montants de sortie en clair, alors que l'hybride est une vraie tx RingCT (sorties redevenues des engagements de Pedersen + preuves de portée). Seuls les montants des entrées PQ sont révélés.
  - **Deux bugs trouvés en suivant l'hybride** : (1) `get_tx_fee` calculait le frais depuis les montants révélés dès qu'une entrée PQ était présente → pour une hybride (entrées ring et sorties à 0) il lisait **tout le total transparent comme frais** ; exige désormais `RCTTypeNull`. (2) `check_tx_outputs` acceptait d'emblée toute tx ayant une entrée PQ, l'exemptant de la règle « v2 = montants nuls » et de l'échelle de types rct → correct pour la forme transparente, **dangereux pour l'hybride** qui est une tx RingCT et doit subir toutes les règles ; exige désormais `RCTTypeNull` aussi.
  - **Test `src/crypto/pq_hybrid_test.cpp`** (pilote les vrais `rct::genRctSimple` / `rct::verRctSemanticsSimple`) : la démonstration de CRIT-2 (b/c/d/e acceptent un montant gonflé 1000×, seul b2 rejette) ; le cas masque identité ; une tx RingCT classique inchangée ; l'équilibre hybride qui **échoue** si le terme transparent est laissé à 0 (preuve que le check contraint réellement, au lieu de passer à vide) ; rejet de tout total transparent faux (+1, ×2, −1).
...
- **Le gate du 8 septembre a été honoré, pas contourné.** Procédure de
  ...
  branche jetable, submodule sur 0.16.0, rebuild liboqs + `cncrypto`, **rejeu du vecteur figé**.
  ✅ **Les 4 groupes reproduisent bit pour bit**, et les 15 tests standalone passent. L'arbre a
  été restauré sur 0.15.0 et re-vérifié **avant** que la décision d'upgrader ne soit demandée.
- **Pourquoi ça passe — lu dans le source des deux versions, pas déduit** : 0.15.0
  (`pqcrystals-dilithium-standard`) fait `randombytes(seedbuf, SEEDBYTES)` — **un appel, 32 o** ;
  0.16.0 (`mldsa-native`) fait `mld_randombytes(seed, MLDSA_SEEDBYTES)` — **un appel, 32 o**. Les
  deux enchaînent sur la dérivation `ξ → (pk, sk)` **spécifiée par FIPS 204**, déterministe. La
  crainte du gate était fondée en principe et **infirmée en fait** : `mldsa-native` a changé
  l'implémentation, pas le motif de consommation d'aléa.
- **Risque macOS ARM64 écarté, sur les DEUX versions.** Nos livrables incluent des binaires ARM64
  (daemon `v2.0.3`, GUI `v2.0.2-gui`), donc une clé BQ créée là-bas doit dériver bit-identique.
  Vérifié par lecture : en **0.16.0** les trois backends (`ref`/`x86_64`/`aarch64`) partagent un
  `sign.c` **au hash identique** — aucun fichier commun ne diffère, l'aarch64 n'ajoute que des
  noyaux arithmétiques en assembleur. En **0.15.0** — plus subtil — **il n'existe aucun backend
  aarch64** : ARM tombe sur `ref` tandis que x86_64 prend `avx2`, donc deux implémentations
  réellement distinctes (`sign.c` de hash différent) ; mais le **prologue aléa → (rho, rhoprime,
  key) est byte-identique** entre les deux (`diff` sans différence). ⚠️ **Écarté par LECTURE, pas
  par mesure** : aucun test n'a tourné sur ARM. Action ouverte : `pq_vector_test` sur le runner
  `macos-14` de la CI GUI.
- **Correction au dossier** : `M-11_liboqs_maturity_2026-09-08.md` affirmait « aucun audit tiers
  publié ». **Faux** — Trail of Bits a revu des **portions** de liboqs (2024, rapport avril 2025),
  la page sécurité d'OQS le dit. Ça ne lève pas la condition (revue partielle, et OQS maintient
  *après* elle que le niveau d'audit reste insuffisant), mais un fichier d'audit ne doit pas
  porter une affirmation fausse. Barré et corrigé en place.
- **UPGRADE PRISE** : submodule `97f6b86…` (0.15.0) → **`5a1a854b0dc9f2141bdc771c555ee60c37950183`**
  (tag **0.16.0**). Rebuild complet, **15/15 standalone verts**, `unit_tests` verts. Références au
  pin mises à jour là où elles sont **opérationnelles** : `README.md` (procédure M-10 lue par un
  packageur), `src/crypto/pqc.h`, `docs/whitepaper_v1.4.md`, et la ligne M-10 de ce fichier.
- **Le vecteur figé vaut désormais pour DEUX versions.** `pq_vector_test.cpp` disait « Frozen
  against liboqs 0.15.0 » ; la bonne correction n'est pas « 0.16.0 » mais **« identique sur les
  deux »** — c'est un gain : le prochain bump aura deux points de référence au lieu d'un. **Les
  valeurs n'ont pas été touchées** (règle absolue de ce fichier).
- **Ce que l'upgrade ne change PAS** : **M-11 reste bloquant pour HFv16** (disclaimer « not for
  production » intact, aucune validation FIPS 140-3, posture hybride non formalisée, re-audit
  constant-time non fait). Toujours **pas de `keypair_derand` pour SIG** en 0.16.0 → le
  contournement par le hook `randombytes` reste nécessaire et le risque reste structurel pour les
  bumps futurs. Aucune correction 0.16.0 ne nous concernait (XMSS, CROSS, FrodoKEM, `encaps_derand`
  — rien de ce que nous compilons ou appelons) : le gain est de **ne plus être EOL**, pas un
  correctif. Chaîne live intacte (hf 15, PQC inerte).

### 12 Septembre 2026 (suite) — spec 2e livrée : binding C-1 + S1b, et fin du résiduel CRIT-3
- **Spec d'abord** (`docs/audit/spec_2e_bq_address_auth_binding.md`, écrite sur la base de
  `design_2c`/`design_2d`), puis implémentation à la lettre. Décisions actées : suppression de la
  signature de compte `0x06`, **T1** (identité ML-DSA par (sub)adresse), **T2** imposée par le
  wallet, **T4** (octet de version + champ d'entrée typé). **T3 hors périmètre**, sa place réservée.
- **Format d'adresse BQ** : `marker(1) ‖ auth_ver(1) ‖ B ‖ A ‖ kem_pk ‖ auth_commit(32)` = **1282 o
  → 1770 caractères** (mesuré). `auth_ver` est placé **avant tout champ interprétable** pour qu'un
  parseur rejette une version inconnue sans avoir rien lu ; `auth_commit = Keccak("HRG_BQ_ADDR_AUTH_v1"
  ‖ auth_ver ‖ auth_pk)`, le type étant DANS le préimage (deux types ne peuvent pas collisionner).
  Le consensus traite l'engagement comme **opaque** — c'est ce qui rend T3 ajoutable sans hard fork.
- **Le binding C-1, enfin défini** : `bind_tag v2 = Keccak("HRG_PQ_BIND_v2" ‖ auth_ver ‖ P' ‖
  auth_commit ‖ auth_blind)`, publié à la création par l'émetteur (qui lit l'engagement dans
  l'adresse), recalculé à la dépense depuis la clé révélée. **`auth_blind` est indispensable et
  n'était pas dans design_2d** : sans lui, révéler `auth_pk` une seule fois permettrait de balayer
  toute la chaîne et d'identifier **toutes** les sorties jamais envoyées à cette subaddress — une
  destruction rétroactive de B3. C'est une image **à sens unique** de `ss` (`ss` n'est jamais révélé).
- **Résiduel CRIT-3 FERMÉ**, avec contrôle négatif explicite : un émetteur à qui l'on **donne** `x'`
  (ce que Shor lui rendrait) et qui détient `ss` satisfait (b)(b2)(d)(d2)(e) et **échoue sur (c)
  seul**. (c)+(d) et (d2) sont **complémentaires, aucun n'est redondant** : sans d2 un émetteur
  engage son propre engagement ; sans c/d un quantique forge `x'`.
- **Résidu §3.4, assumé et testé des DEUX côtés** : le consensus ne voit pas l'adresse, donc il ne
  peut pas vérifier que l'émetteur a engagé le bon engagement. Contrôle **obligatoire côté wallet
  au scan** : une sortie mal engagée n'est **jamais créditée ni affichée**. Les deux moitiés sont
  testées (validateur accepte / wallet refuse), plus un contrôle positif.
- **T2 appliquée, pas conseillée** : `m_pq_spent_subaddresses` (cache, `BOOST_CLASS_VERSION` 31→32,
  marquage **à la construction**), R-a (refus explicite dans `get_pq_subaddress_as_str`), R-b
  (change vers une subaddress BQ **neuve**), R-c (refus de fusionner deux subaddresses BQ sans
  opt-in). **Constat trouvé en codant** : le change d'une dépense BQ allait sur `{major,0}` — soit
  l'adresse BQ **primaire** (pire cas de liabilité T1) pour `major == 0`, soit une sortie
  **classique B...** pour `major > 0`, c'est-à-dire une reconversion silencieuse en monnaie
  vulnérable au quantique. Les deux sont corrigés.
- **Budget `tx_extra` : 2 → 7 sorties BQ par dépense** (mesuré sur de vraies transactions), la
  suppression des 5262 o de `0x06` levant la contrainte mesurée en `design_2b §9`. `design_2d §5`
  estimait 6 à 7.
- **Deux défauts latents trouvés en chemin** : (1) `cryptonote_boost_serialization.h` n'avait jamais
  été étendu pour `mask` (CRIT-2) ni `owner_sig` (CRIT-3) → une dépense BQ dans un **cache de
  wallet** revenait amputée ; (2) `pq_auth_commit` n'était pas réhydraté au rechargement → un wallet
  BQ rouvert rendait une adresse **différente** de celle distribuée. Corrigés.
- **Chiffres mesurés, jamais recopiés de la spec** : 1282 o / 1770 car. / pin wire **5512 o** /
  7 sorties. Préfixe « BQ » vérifié sur **2000 charges aléatoires × 2 markers** avec `auth_ver=0x01`
  (test permanent, condition d'allocation de toute future version).
- **Suite standalone = 15 tests, tous verts** (nouveau `pq_auth_binding_test`) ; `unit_tests` verts ;
  daemon + wallet + simplewallet compilent ; smoke CLI réel OK. Vecteurs figés ajoutés (clé
  d'identité de subaddress, `auth_commit`, `auth_blind`, `bind_tag` v2) ; **sentinelles liboqs
  inchangées au bit près**. `HF_HEIGHT_PQ` inchangé, chaîne live intacte, **aucune migration**.
- **Reste bloquant HFv16** : M-11 (maturité liboqs). T3 reste une direction de recherche.

### 12 Septembre 2026 — CRIT-4 corrigé : la racine PQ a sa propre seed (décision R2a)
- **Rapport de design d'abord** (`docs/audit/design_2c_pq_root_shor_resistance.md`), puis
  implémentation. Le rapport **réfute son hypothèse de départ** en vérifiant le code : dériver la
  racine PQ « directement de la seed sous un domaine de hash séparé » n'apporte **rien**, parce que
  **la seed 25 mots EST `m_spend_secret_key`** — `generate_keys` en `recover=true` fait
  `sec = recovery_key` puis `sc_reduce32`, **sans hachage** (`crypto.cpp:159-166`), et `get_seed`
  réencode littéralement la clé de dépense en mots (`wallet2.cpp:1450-1453`). Parallèle ou à
  travers, les deux branches partent du nœud que Shor donne. **Fermer CRIT-4 exigeait de l'entropie
  qui n'existait pas dans le wallet.**
- **Décision R2a — deux mnémoniques** : `account_keys::pq_root`, 32 octets d'entropie indépendante
  tirés du CSPRNG système, sauvegardés par **leur propre liste de 25 mots**. `get_pq_root_secret`
  renvoie ce champ (ou `nullptr` — **jamais de repli sur la clé de dépense**). La dérivation Ed25519
  n'est pas touchée : **une même seed classique rend la même adresse B...**, et les binaires publiés
  (v2.0.3, GUI v2.0.2-gui) restaurent toujours correctement la moitié classique.
- **Fail-closed partout.** Restaurer avec `--bq-wallet` sans seed BQ → **refus**, appliqué dans
  `wallet2::generate` (pas seulement la CLI) : une racine neuve donnerait une adresse BQ valide qui
  n'est pas celle des fonds — M-4 en silencieux. Un `.keys` avec `pq_keys` mais sans `pq_root` (wallet
  BQ d'avant le correctif) est **refusé au load**. Point non couvert par le rapport, tranché et
  documenté ; corollaire assumé : « attacher une racine neuve à une seed classique » n'existe pas.
- **Taille de la seed BQ : 32 o / 25 mots** (le rapport laissait 16 o / 13 mots ouvert). 16 o
  plafonnent Grover à 2⁶⁴, **sous** le budget des primitives ; et `words_to_bytes(…, secret_key&, …)`
  traite déjà 32 octets verbatim **sans `sc_reduce32`** → zéro changement dans la couche mnémonique.
- **Les trois points mécaniques du rapport, tenus** : `pq_root` ajouté **en fin** du flux chacha20
  positionnel (`account.cpp:87-125`) ; `BOOST_CLASS_VERSION(account_keys)` 1 → 2 ; cold-sign v4
  jamais publié donc libre. Champ KV nommé optionnel comme `pq_keys` → **un wallet classique n'émet
  rien, son `key_data` est inchangé** ; pas de bump du format `.keys` lui-même.
- **`pq_vector_test` — régénération délibérée et bornée**, exactement le protocole annoncé :
  régénérés = adresse BQ + les 4 empreintes de clés de compte, sous une nouvelle constante
  `V_PQ_ROOT` **choisie différente de `V_RECOVERY_KEY`** (sinon le vecteur passerait encore contre le
  code vulnérable) ; **inchangés et re-mesurés pour le prouver** = `V_RAW_*`, `V_OUT7_*`, `V_BIND_TAG`,
  revenus **identiques au bit près**. C'est ce qui sépare « on a changé l'entrée » de « la dérivation
  a bougé » — et c'est toute la raison d'être du fichier.
- **Nouveau test `pq_root_restore_test`** (code réel des deux côtés) : deux seeds distinctes ; restaurer
  avec les deux reproduit B..., BQ, les clés ML-KEM/ML-DSA et les subaddresses ; **la propriété CRIT-4
  elle-même** — même seed classique + seed BQ différente → **même** B..., **autre** BQ (assertion
  inexprimable avant) ; restauration sans seed BQ refusée ; un vrai paiement vers l'adresse BQ + une
  subaddress est trouvé par le wallet **restauré**, qui reproduit le bind tag on-chain (donc peut
  dépenser), tandis qu'une mauvaise seed BQ ne trouve rien ; la racine survit à store/load.
- **Défaut trouvé en pilotant la vraie CLI** (les tests unitaires ne pouvaient pas le voir — leurs
  wallets tournent « unattended », sans chiffrement mémoire) : le premier jet affichait la seconde
  seed **depuis la racine chiffrée en mémoire**, soit **25 mots valides restaurant un AUTRE wallet**.
  `get_pq_seed` refuse désormais quand les clés sont verrouillées (garde `pq_secrets_usable()`, même
  famille que le défaut ÉLEVÉ de `design_2b §8.5`) ; `simplewallet` déverrouille à la création, la
  commande `seed` était déjà sous `SCOPED_WALLET_UNLOCK`. Cas pinné par un test.
- **Migration : aucune.** `HF_HEIGHT_PQ = 2 000 000`, chaîne en hf 15 → aucune sortie BQ n'a jamais
  existé sur le mainnet ; seuls des wallets de test locaux sont invalidés (même précédent que la
  migration FIPS de l'Étape 9).
- **Effet sur CRIT-3** : la ligne « tiers quantique » de la matrice passe à **non** (sans la racine
  cassée, il n'obtient plus `ss`). **L'émetteur quantique reste ouvert** — `x'` tombe par Shor sur `P'`
  quelle que soit la racine. Fermeture = **S1b**, à livrer **avec** le binding C-1 ; l'ordre était
  imposé, S1b dérivant sa clé de la racine.
- **Suite standalone = 14 tests, tous verts.** `HF_HEIGHT_PQ` inchangé, chaîne live intacte.
  Détail : `docs/audit/CRIT-4_pq_root_was_the_spend_key_2026-09-12.md` + `design_2c` §8.

### 11 Septembre 2026 — subaddresses BQ (décision 4, B3) + CRIT-3 + CRIT-4 (ouvert)
- **Subaddresses BQ livrées** (`1d901e541` code, `a88d5297c` doc) — `docs/audit/design_2b_bq_subaddresses.md` §7–10. Une paire ML-KEM-768 par subaddress, dérivée de `get_pq_root_secret()` (**pas** de la clé de vue `a` : une clé ML-KEM dérivée de `a` donnerait le pouvoir de dépense au détenteur de la clé de vue). `(0,0)` = adresse BQ primaire, dérivation M-4 inchangée (`pq_vector_test` intact). Tag de sélection 8 o = `fp(kem_pk) XOR pad(d, i)` → recherche **O(1)** dans `m_pq_subaddresses` (empreinte → index, en mémoire seulement, reconstruit depuis la seed ; rien de nouveau dans `.keys`). Champ `0x07` = `[output_index | sel_tag | ct]` ; règle consensus `check_pq_output_field_indices` (index ML-KEM et binding dans les bornes, uniques, mêmes sorties). Marqueur d'adresse subaddress `0x35` (rend toujours « BQ »). `simplewallet` : `address bq <index>`. Cold-sign : `pq_source_ct` porte l'index de subaddress, la machine froide le vérifie.
- **Défaut ÉLEVÉ préexistant corrigé au passage** : en mode par défaut (`AskPasswordToDecrypt`), `kyber_sk` est chiffré en mémoire pendant le refresh → l'ancienne détection BQ décapsulait avec des octets brouillés → **aucun wallet interactif ordinaire ne détectait jamais de sortie BQ**. Désormais le pré-filtre par tag ne demande que la clé de vue ; le mot de passe n'est demandé qu'en cas de correspondance.
- **Budget `tx_extra` mesuré** ~~: la ML-DSA de compte (5262 o) est **par tx**, pas par sortie (erreur du rapport 2b). Max sorties BQ : **7** en paiement B→BQ (7957 o), **2** en dépense BQ transparente (7570 o) — un paiement BQ + un change BQ. À trancher avant HFv16 (relever `MAX_TX_EXTRA_SIZE_PQ` ou sortir la signature de compte de `tx_extra`).~~ — **CHIFFRE PÉRIMÉ, voir spec 2e (12 septembre, section ci-dessus).** La mesure du 11 septembre était juste à sa date ; la question « à trancher » a été tranchée par la **seconde** option : la signature de compte `0x06` est **supprimée** de `tx_extra`. Les 5262 o rendus portent la dépense BQ transparente de **2 à 7** sorties, mesuré sur de vraies transactions (`pq_subaddress_test`) — donc **7 des deux côtés**, `MAX_TX_EXTRA_SIZE_PQ` inchangé à 8192.
- **CRIT-3 — CORRIGÉ contre un émetteur classique** (`678c170b9`, `docs/audit/CRIT-3_sender_can_reclaim_bq_output_2026-09-11.md`). Une dépense BQ transparente n'exigeait que `ss` (secret ML-KEM **partagé** — l'émetteur l'obtient en encapsulant), le montant et le masque (déduit de la clé de tx de l'émetteur), plus une ML-DSA de compte non liée (C-1). **L'émetteur pouvait redépenser toute sortie BQ qu'il avait payée.** Fix : check consensus **d2** — `txin_to_key_pq.owner_sig`, signature Ed25519 par la clé unique `x' = H_s(d,i) + b (+m) + t`, qui exige la clé de dépense du destinataire. **Résiduel : un émetteur QUANTIQUE** (Shor sur `P'`) n'est pas arrêté → fermer ce cas exige une clé PQ détenue par le seul destinataire dans le binding (piste : ML-DSA par subaddress dans l'adresse, coût = liabilité des dépenses d'une même subaddress) → **bloquant HFv16, à trancher avec C-1**. NB : l'affirmation M-11 « la dépense reste gardée par dlog Ed25519 » était FAUSSE pour la dépense BQ jusqu'à ce fix.
- **Deux défauts de l'hybride (décision 3, `bab8ca00e`) corrigés avec CRIT-3** : (1) les signatures PQ étaient calculées avant la mise à zéro RingCT des montants → **toute tx hybride était invalide** ; (2) clés de signature PQ indexées par position `vin` avant le tri → mauvaise entrée. `pq_hybrid_test` ne passait jamais par `construct_tx` + validateur.
- **Épingle wire `txin_to_key_pq`** (unit test) : périmée depuis CRIT-2 (+32 o mask, jamais mise à jour), mise à jour à 5479 o (+64 o `owner_sig`).
