...
### Étape 5 (PROCHAINE) — vrai plumbing des clés BQ (BLOQUÉ sur le format d'adresse)
**Item ouvert principal** : `account_public_address` ne peut PAS encore porter une clé publique Kyber768 de **1184 octets** (il n'a que les clés spend/view Ed25519 de 32 octets) → **les adresses BQ... ne sont pas constructibles** tant que ce plumbing n'a pas atterri. Tant que c'est le cas, l'Étape 4 encapsule contre une **clé destinataire jetable** (encaps throwaway, cf. TODO dans `cryptonote_tx_utils.cpp`) — code exerçable mais PAS le schéma final.
À faire en Étape 5 : (1) définir le format d'adresse BQ... portant la clé Kyber768 (+ Dilithium3) et son encodage base58 sous le préfixe `0x3C11` ; (2) parsing wallet des adresses BQ... + positionner `tx_destination_entry::is_pq` ; (3) stockage des clés PQ du destinataire dans `account_keys`/`account_public_address` (ou structure dédiée) ; (4) câbler le scan côté `wallet2` (récupérer la kyber_ciphertext depuis `tx.extra`, `pqc_stealth_decaps`, recalculer le tweak `H_s(ss)·G` pour retrouver la one-time key) ; (5) passer la vraie `hf_version` aux call sites (caveat 2). Toujours additif et gardé par HFv16.
