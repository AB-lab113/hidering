...
- src/checkpoints/checkpoints.cpp — ancres mainnet h=2939/5000/11000/16000/20000, hash-enforced depuis `934b43b01` (6 juin 2026) ; branches TESTNET/STAGENET **vidées** (M-1, 13 juin) — anciennes ancres Monero héritées retirées (auraient halté tout testnet/stagenet HRG sous enforcement)
...
> **État du backlog 10 juin (série MOYEN, vérifié 13 juin 2026)** — **FAITS (11/13)** : M-3 `.keys` sans MAC (risque accepté), M-5 seeds Tor/i2p purgés (`eee5dec50`), M-6 DNSSEC seeds (`fbf0e0600`), M-7 padding 2500 = non-problème (`a220b960d`), M-9 rapidjson CVE = faux positif, M-10/M-11 liboqs (`174c4beac`), **M-2** chiffres de supply corrigés → circulant max = **18 000 000,00 exact** (NUMS genesis hors `already_generated_coins`, `blockchain.cpp:4427`) dans CLAUDE.md + GENESIS_PROOF.md + whitepaper site (`b3f427705` + `f5c5d0ecd`), **M-12** `test_tx_utils.cpp` migré ML-DSA sig 3293→3309 (`592af786a`), **M-1** checkpoints testnet/stagenet Monero hérités **vidés** + guard underflow `pt.first-2` (`checkpoints.cpp` branches TESTNET/STAGENET → `return true` vides ; `blockchain.cpp:4739` clamp `>=2?:0`) — mainnet byte-identique (ancres h=2939…20000 intactes), testnet boote sans halt (smoke OK), M-13 RPC pool localhost, **M-14 OBSOLÈTE** (script `deploy-checkpoint-h11000.sh` à hash non vérifié → jamais commité, remplacé par la procédure standard `git pull + make -j$(nproc)` sur le VPS — cf. INFRASTRUCTURE VPS ; finding sans objet). **OUVERTS (2/13)** : **M-4** restore depuis seed 25 mots **perd les fonds BQ** (clés PQ aléatoires non dérivées du seed, `account.cpp:298`) — **décision de modèle de backup à trancher AVANT HFv16** ; **M-8** « ring = ensemble total » si `num_outs ≤ ~97` (`wallet2.cpp:9531-9540`) — non déclenché (chaîne >11K outputs), **item de surveillance au launch** de toute chaîne relancée vierge.
